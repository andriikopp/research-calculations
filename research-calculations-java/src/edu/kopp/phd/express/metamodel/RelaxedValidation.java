...
public class RelaxedValidation {
    public static final double[] BPMN_ATTRIBUTES        = {1, 0, 0};
    public static final double[] DFD_ATTRIBUTES         = {0, 1, 0};
    public static final double[] ARISeEPC_ATTRIBUTES    = {1, 0, 1};
    public static final double[] IDEF0_ATTRIBUTES       = {0, 0, 1};
    ...
    static {
        ...
        ATTRIBUTES.put("CF", BPMN_ATTRIBUTES);
    }
    ...
    public static double validate(Model model, String type, boolean log) {
        ...
        double UI = getUndefinedInputs(model, isRegulationsConsidered).size();
        double UO = getUndefinedOutputs(model, isRegulationsConsidered).size();
        ...
        if (log) {
            System.out.printf("%.2f\t%.2f\t%.2f\t%.2f\t%.2f\t%.2f\t%.2f\t%.2f\t%.2f\t%.2f\t%.2f\t",
                    ...
                    attributes[2] * UR, attributes[2] * UI, attributes[2] * UO, attributes[2] * UT);
        }

        double defectDensity
                = attributes[0] * (MSE + MEE + MSG + MJG)
                + attributes[1] * (BHS + MS + GHS)
                + attributes[2] * (UR + UI + UO + UT);
    ...
    private static List<Function> getUndefinedInputs(Model model, boolean isRegulationsConsidered) {
        List<Function> functions = new ArrayList<>();

        for (Function function : model.getFunctions()) {
            if (isRegulationsConsidered) {
                if (function.getInputs() == 0 || function.getRegulations() == 0) {
                    functions.add(function);
                }
            } else {
                if (function.getInputs() == 0) {
                    functions.add(function);
                }
            }
        }
    ...
    private static List<Function> getUndefinedOutputs(Model model, boolean isRegulationsConsidered) {
        List<Function> functions = new ArrayList<>();

        for (Function function : model.getFunctions()) {
            if (function.getOutputs() == 0) {
                functions.add(function);
            }
        }
